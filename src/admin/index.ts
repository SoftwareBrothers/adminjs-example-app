...
import {
  Database as MikroormDatabse,
  Resource as MikroormResource
} from '@adminjs/mikroorm';
import {
...
} from "../sources/typeorm/resources";
import {
  CreateOwnerResource,
  CreateSellerResource,
  CreateCarResource
} from '../sources/mikroorm/resources';
import {
  CreatePostResource,
  CreatePublisherResource,
  CreateProfileResource
} from '../sources/prisma/resources'
...
AdminJS.registerAdapter({ Database: MikroormDatabse, Resource: MikroormResource });
...
export const createAdmin = async () => {
  ...
  if (!admin) {
    await AdminModel.create({ email: ADMIN.email, password: await argon2.hash(ADMIN.password) });
  }
}
